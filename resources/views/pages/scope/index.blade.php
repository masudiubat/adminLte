...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <div class="add-user-btn float-right" style="width:20%">
                    <button type="button" class="btn btn-block btn-outline-success" data-toggle="modal" data-target="#NewScopeModal">Scopes</button>
                </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        @foreach($scopes->reverse() as $key => $scope)
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td>{{ ucfirst($scope->name) }}</td>

                            <td>
                                <a onclick="event.preventDefault(); editScope('{{ $scope->id }}');" href="#" class="btn btn-xs btn-secondary" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>
                                <a href="#" onclick="event.preventDefault(); deleteScope('{{$scope->id}}');" class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></a>
        ...
        <div class="modal fade" id="NewScopeModal">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="card card-primary">
                            <!-- form start -->
                            <form id="newScopeForm" action="{{route('scope.store')}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="card-body">
...
<script>
    function editScope(id) {
        var url = "{{url('/scope/edit')}}/" + id;
        console.log(url);
        $.ajax({
            url: url,
            method: "GET",
        }).done(function(data) {
            $('.AddEditForm').html(data.editScope);
